...
Просится положить даг рядом с конфигом, в `er_config.py`. Нельзя: `_process_modules`
(`airflow/models/dagbag.py`) собирает даги не только из `__dict__` разбираемого файла, но
и добавляет `DagContext.autoregistered_dags` — а туда попадает всё, созданное декоратором
`@dag` за время разбора **текущего** файла. `er_config.py` импортируют `er_sync.py`,
`er_export.py` и этот модуль; окажись `@dag` в конфиге, при разборе `er_sync.py` импорт
выполнил бы декоратор, и даг привязался бы к чужому файлу. Потом ещё раз — при разборе
`er_export.py`.

Правило общее: **модуль, который импортируют DAG-файлы, сам DAG-и создавать не должен.**
По той же причине валидация переехала из `er_sync.py` в `er_config.py` — за хелпером
не стоит импортировать файл с дагом.

Освободить имя `er_config.py` под этот даг, переименовав конфиг в `er_common.py`, тоже
не выйдет: в проде наши файлы разворачиваются в пакет
`CI06932748.analytics.datalab.export_er/`, где `er_common.py` уже занят легаси-модулем
старого ER, и из него импортирует живой легаси-даг.
