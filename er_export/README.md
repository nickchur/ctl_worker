...
| `er_core.py` | **Ядро системы**. Содержит `TaskGroup` с логикой выгрузки, формирования метаданных, упаковки в ZIP и отправки в Kafka. |
| `er_sync.py` | **DAG синхронизации**. Переносит настройки из таблицы ClickHouse в Airflow Variable/S3 для ускорения работы фабрики. |
| `er_config.py` | **Конфигурация**. Настройки подключений, маппинг типов данных, системные константы и обязательные поля. |
