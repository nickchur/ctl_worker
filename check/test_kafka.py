...
сообщения) без какого-либо прикладного пайплайна:
  ...
  📤 tools_test_kafka_snd — шлёт одно XML-сообщение `TransferFileCephRq` в топик.
     ...
     файл не попадёт в счётчики лимитов маршрута, а пул `tfs_send` этот таск не берёт
     (он живёт в `TOOLS_POOL`, а двух пулов у таска не бывает) — то есть возможна
     отправка одновременно с `export_er_sender`. Для разовой проверки это допустимо,
...
Режимы приёма (`mode`):
  ...
  • `wait` — `ConsumeFromTopicOperator`: синхронно опрашивает топик на воркере, triggerer
    ...
    `tfs_receipts_sync` — единственным штатным потребителем этого топика. Сообщение,
    ...
    `wait` можно только при остановленном `tfs_receipts_sync`.
