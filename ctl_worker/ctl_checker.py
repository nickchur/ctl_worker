"""### 🔍 DAG: Проверка API CTL (Change Tracking & Loading)

Этот DAG предназначен для **диагностики и тестирования API системы CTL**. Он позволяет отправлять произвольные HTTP-запросы к внутренним эндпоинтам CTL, просматривать ответы и отлаживать интеграции.

---

#### 🎯 Назначение

- Тестирование доступности и корректности API.
- Отладка параметров запросов.
- Проверка структуры данных (сущностей, workflow'ов, загрузок).
- Поддержка разработчиков и администраторов ETL-процессов.
...
| Задача | Описание |
|-------|----------|
| `chk_ctl_api` | Выполняет HTTP-запрос к API CTL с заданными параметрами и выводит результат. |

---

#### ⚙️ Параметры DAG (через UI)

DAG поддерживает гибкую настройку через интерфейс Airflow (`params`). Все поля заполняются при ручном запуске.

| Параметр | Описание | Примеры |
|---------|----------|---------|
| `url` | Шаблон URL API CTL. Поддерживает подстановку `{lid}`, `{wid}`, `{eid}`, `{limit}`. | `/v4/api/entity/{eid}/child`, `/v4/api/loading/{lid}` |
| `method` | HTTP-метод запроса. | `GET`, `POST`, `PUT`, `DELETE` |
| `eid` | ID сущности (entity). Автозаполнение из `ctl_entities`. | `941010000` (DATAMART#...) |
| `wid` | ID workflow. Автозаполнение из `ctl_workflows`. | `12345` |
| `lid` | ID загрузки. | `54321` |
| `limit` | Лимит результатов. | `100` |
| `data` | JSON-данные для POST/PUT запросов. | `{"status": "RUNNING"}` |

---

#### 🏷️ Метаданные

- **ID DAG:** `tools_ctl_check_api`
- **Теги:** `['EDP.ETL', 'tools', 'CTL']`
...
| Эндпоинт | Описание |
|----------|----------|
| `/v5/api/info` | Информация о системе и профиле |
| `/v4/api/permission` | Проверка прав доступа |
| `/v4/api/wf/{wid}` | Данные о workflow |
| `/v4/api/loading/{lid}` | Статус и детали загрузки |
| `/v4/api/entity/{eid}/child` | Иерархия сущностей |
| `/v4/api/entity/{eid}/statval/all` | Статистики по сущности |
| `/v4/api/wf/{wid}/loading` | История загрузок workflow |

---

#### 🔄 Использование

1. Запустите DAG вручную через UI Airflow.
2. Заполните параметры (URL, метод, ID).
3. Проверьте результат в логах и заметках.

> 💡 *Примечание:* Этот DAG — диагностический инструмент. Не используется в автоматических процессах.  
> Результаты логируются в Airflow и доступны в **Task Logs**.
"""
