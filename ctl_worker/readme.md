...
*2026-09-04 13:00 MSK · v1.9 · Nick Churkin · [NSChurkin@sber.ru](mailto:NSChurkin@sber.ru)*
...
|---|---|
| «паузу поставил `<кто>` `<когда>`» | осознанное решение человека — видно по событию `paused` (веб) или `cli_dag_pause` (командная строка) в журнале метабазы |
| «пауза с создания» | записи в журнале нет: фабрика создаёт даг запаузенным, если воркфлоу не на расписании в CTL (`is_paused_upon_creation = not w['scheduled']`) |
...
раз за сутки» считалось запросом, а не поиском по логам. Загрузки, застрявшие в этом
состоянии, монитор показывает списком с возрастом (см. выше).
...
**Заметка «⏸️ Ждут снятия паузы».** Рядом со сводкой SLA монитор перечисляет активные
загрузки, чей даг стоит на паузе, с возрастом от последней смены статуса. Такая загрузка
не поедет: сенсор её пропускает (см. ниже), а сама пауза снимается человеком. Без этой
строки «загрузка ждёт вторые сутки» видно только чтением логов сенсора минута за минутой —
на снимке боевых данных таких загрузок нашлось 28.
