"""### 🛠️ Модуль: Утилиты для работы с S3 (plugins/s3_utils.py)

Этот модуль предоставляет **расширенные функции для взаимодействия с Amazon S3** в рамках системы CTL. Он используется DAG'ами для копирования, архивации, потоковой обработки и управления жизненным циклом объектов в хранилище.

---

#### 🔧 Основные возможности

1. **Управление TTL (временем жизни)**:
   - `s3_set_ttl()` — настраивает автоудаление объектов.
   - `s3_get_ttl()` — получает текущие правила жизненного цикла.
   - `s3_del_ttl()` — удаляет правила.

2. **Операции с бакетами**:
   - `s3_create_bucket()` — создаёт бакет, если не существует.
   - `s3_get_buckets()` — возвращает список всех бакетов.
   - `s3_bucket_size()` — рассчитывает общий объём данных и количество объектов.
   - `get_s3_list()` — ленивая инициализация списка всех бакетов по всем S3-соединениям; результат кешируется. Вызывайте явно вместо обращения к модульной переменной `s3_list`.

3. **Работа с путями**:
   - `s3_path_parse()` — разбирает S3-путь на компоненты (`conn_id`, `bucket`, `key`, `prefix`, `mask` и др.).

4. **Копирование и перемещение**:
   - `s3_to_s3()` — копирует объект между бакетами (с опциональным GZIP-сжатием).
   - `s3_move_s3()` — перемещает объект с удалением исходника.
   - Поддержка `.done`-файлов через `s3_done()`.

5. **Обработка архивов**:
   - `s3_from_zip()` — потоковое извлечение файлов из ZIP без сохранения на диск.
   - Автоматическое сжатие при извлечении (`compress=True`).

6. **Потоковая передача**:
   - `s3_IterStream` — оборачивает генератор байтов в file-like объект.
   - `s3_gzip_stream()` — потоковое сжатие в GZIP.
   - Защита от OOM: контроль буфера и предупреждения при превышении порога.

7. **Поиск объектов**:
   - `s3_keys()` — возвращает список объектов по маске (с поддержкой `*`, `?`), фильтруя служебные файлы.
...
- **Используемые библиотеки**:
  - `airflow.providers.amazon.aws.hooks.s3` — основной хук.
  - `stream_unzip` — для потокового чтения ZIP.
  - `boto3`, `botocore` — низкоуровневое взаимодействие с S3.

- **Безопасность памяти**:
  - Все операции с большими файлами — потоковые.
  - Жесткие лимиты буфера (до 512 МБ) с защитой от `MemoryError`.

- **Логирование**:
  - Подробные логи через `getLogger("airflow.task")`.
  - Информация о скорости, размерах, времени выполнения.

- **Ленивая инициализация бакетов**:
  - При импорте модуля перечисление S3-бакетов не выполняется.
  - Список доступен через `get_s3_list()` — вызов при первом обращении, далее из кеша.

- **Гибкость**:
  - Поддержка разных типов соединений (`conn_id`).
  - Совместимость с Airflow Dataset и XCom.
...
| DAG | Использует |
|-----|-----------|
| `ctl_tfs.py` | `s3_move_s3`, `s3_from_zip`, `s3_keys`, `s3_done` |
| `ctl_loader.py`, `ctl_monitor.py` | `s3_set_ttl`, `s3_create_bucket` |

---

> 💡 *Примечание:* Все функции безопасны для использования в Airflow-воркерах.  
> Предназначены для интеграции с системой CTL, но могут быть переиспользованы в других ETL-процессах.
"""
