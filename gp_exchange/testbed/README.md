...
Все пять дефектов исправлены; `55_seed_log.sql` оставлен как удобный способ задать
начальную отсечку, но обязательным он больше не является.

1. **Первая загрузка потока не стартовала.** `do_branch` соединялся с `gp__exchange_log`
   через INNER JOIN, поэтому поток без записи `IN_log` отсекался, а запись появлялась
   только после обработки — круг замкнут. Теперь LEFT JOIN и нулевая отсечка.
2. **Пустое ветвление оставляло DAG без сигнала.** Все `process_*` уходили в skip,
   `end_task` с `trigger_rule='one_success'` не выполнялся, Dataset не публиковался,
   а сам DAG был зелёный. Добавлена ветка `process__empty`.
3. **Разведка незнакомого потока упала бы на первом же пакете:** `do_branch` возвращал
   `'process__other'` без префикса группы, тогда как задача называется
   `'process.process__other'`.
4. **Автоопределение типов склеивало разные типы одного поля** (`'Double,Int64'`) —
   такого типа нет, `JSONExtract` падал. Исправлено в `sql_fields`.
5. **`CASE` над колонками не работает в ClickHouse 24.8** (`transform` требует
   константных веток) — поток с `fields='*'` не грузился при существующей целевой
   таблице. Переписано на `multiIf`.

## Сценарии проверки

Три сценария, которыми проверялись правки 1–3; повторяются на стенде за пару минут.

| Что проверяем | Как воспроизвести | Чего ждём |
| :--- | :--- | :--- |
| Первая загрузка потока | `ALTER TABLE support.gp__exchange_log DELETE WHERE wf_name = 'vw_ztest'`, `TRUNCATE support.gp_vw_ztest`, новый пакет | поток обрабатывается и без записей в журнале |
| Пустое ветвление | `TRUNCATE support.gp_ue_exchange`, перезапустить `branch` | `process__empty` зелёный, `end_task` зелёный, Dataset опубликован |
| Незнакомый поток | вставить строки с `_gp_name`, которому нет задачи, перезапустить `branch` | ветвление уходит в `process.process__other`, разведка показывает поля и образец |

Полезное наблюдение оттуда же: невалидный JSON (например, с сырым табом внутри строки)
разведку не роняет — поля возвращаются пустыми.
