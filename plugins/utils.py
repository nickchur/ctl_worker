"""### 🛠️ Модуль: Утилиты для Airflow (plugins/utils.py)

Этот модуль предоставляет **набор вспомогательных функций**, используемых в DAG'ах и плагинах системы CTL. Он обеспечивает единый подход к логированию, работе с пулами, обработке данных и взаимодействию с Airflow API.

---

#### 🔧 Основные возможности

1. **Управление пулами выполнения**:
   - `pool_slots()` — динамическое управление количеством слотов пула.
   - `get_current_load()` — получение текущей нагрузки по пулу (queued, running, scheduled).

2. **Логирование и уведомления**:
   - `add_note()` — добавление структурированных заметок в интерфейс Airflow (в DAG или Task).
   - `on_callback()` / `_on_callback()` — универсальный обработчик событий (success/failure/retry) с детальной информацией.
   - Поддержка Markdown-разметки и ограничение длины сообщений.

3. **Работа с данными**:
   - `md5_hash()` — генерация хеша JSON-совместимых структур.
   - `readable_size()` — преобразование байтов/чисел в человекочитаемый формат (`MB`, `GB` и т.д.).
   - `readable()` — универсальный форматтер для разных типов данных (datetime, timedelta, dict и др.).
   - `str2timedelta()` — парсинг строки в `timedelta` (например, `'minutes=5'` → `5 минут`).

4. **Безопасные вычисления**:
   - `safe_eval()` — безопасная оценка математических выражений без использования `eval()`.

5. **Работа с подключениями Airflow**:
   - `get_conns_by_type()` — получение списка conn_id по типу соединения.
   - `get_conn()` — извлечение параметров соединения (host, port, schema и др.).

6. **Работа с базой данных**:
   - `query_to_dict()` — выполнение SQL-запроса к Greenplum и возврат результата в виде списка словарей.

7. **Управление DAG'ами**:
...
- **Интеграция с Airflow**:
  - Использует `@provide_session`, `TaskInstance`, `DagModel`, `Pool` напрямую.
  - Полностью совместим с Airflow 2.x.

- **Безопасность**:
  - `safe_eval()` защищён от выполнения произвольного кода.
  - Все операции с БД используют контекстные менеджеры.

- **Логирование**:
  - Централизованное через `getLogger("airflow.task")`.
  - Детальные сообщения при ошибках, успехе, ретраях.

- **Гибкость**:
  - Поддержка динамических пулов (адаптивное масштабирование).
  - Совместимость с `TaskGroup`, `dynamic task mapping`, `Dataset`.
...
|--------|----------------|
| `add_note`, `on_callback` | Все основные DAG'ы: `loader`, `monitor`, `sensor`, `tfs` |
| `pool_slots`, `get_current_load` | `ctl_test_conn`, `ctl_tfs`, `ctl_loader` |
| `md5_hash`, `readable_size` | Сравнение версий, логирование размеров |
| `str2timedelta` | Парсинг расписаний из конфигурации |
| `safe_eval` | Вычисление временных задержек в тестовых режимах |

---

> 💡 *Примечание:* Все функции протестированы и оптимизированы для работы в production-среде Airflow.  
> Предназначены для повторного использования в других ETL-процессах.
"""
