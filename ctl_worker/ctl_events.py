"""### 🔔 DAG: Обработка событий CTL (Change Tracking & Loading)

Этот DAG отвечает за **публикацию Dataset'ов в Airflow** при изменении данных в системе CTL. Он сравнивает события из CTL с активными загрузками и публикует сигналы для оркестрации DAG'ов.

---

#### 🔧 Основные функции

1. **Получение событий** из CTL за последние N минут.
2. **Сравнение событий** с активными загрузками.
3. **Публикация Dataset'ов** в Airflow:
   - `CTL/wf/{wid}/{wf_name}` — для workflow.
   - `CTL/entity/{eid}/{ename}` — для сущностей.
   - `CTL/{profile}/entities` — алиас для всех сущностей профиля.
4. **Логирование** всех действий через `add_note`.

---

#### ⚙️ Особенности

- **Частота запуска:** каждые 5 минут (`schedule_interval: minutes=5`)
- **Пул выполнения:** `ctl_pool` для API запросов.
- **Механизм:** Dataset API для событийной оркестрации.
...
| Задача | Описание |
|-------|----------|
| `get_events` | Получает события из CTL и публикует Dataset'ы. |
| `set_events` | Формирует и отправляет события как `Dataset` через `outlet_events`. |

---

#### 🏷️ Метаданные

- **ID DAG:** `CTL.<profile>.events`
- **Теги:** `['CTL', '<profile>', 'CTL_agent', 'events']`
...
#### 📊 Публикуемые Dataset'ы

| Dataset | Описание |
|---------|----------|
| `CTL/wf/{wid}/{wf_name}` | Событие для конкретного workflow |
| `CTL/entity/{eid}/{ename}` | Событие для конкретной сущности |
| `CTL/{profile}/entities` | Алиас для всех сущностей профиля |
...
1. **Получение событий** из CTL за последние 5 минут.
2. **Фильтрация** по активным загрузкам и проверка дублей.
3. **Публикация Dataset'ов** через `outlet_events` с метаданными (время, ссылки, статистики).
4. **Логирование** действий в Airflow.

> 💡 *Примечание:* Использует `sensor`-задачу с `poke_interval` для периодической проверки.  
> Поддерживает тестовый режим и ограничение на количество обрабатываемых событий (`MAX_EVENTS`).
"""
