...
Каталог **не зависит от `er_export`**: в проде каталоги разворачиваются по отдельности,
и зависимость от чужого пакета означала бы, что тракт не поднять без выгрузок ER.
Общие хелперы (`add_note`, `on_callback`, `ensure_pool`) берутся из `plugins.utils` —
так же, как в `er_config`.
