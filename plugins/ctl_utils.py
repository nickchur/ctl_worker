"""### 🛠️ Модуль: Утилиты для CTL (plugins/ctl_utils.py)

Этот модуль предоставляет **набор расширенных функций** для взаимодействия с системой CTL, Greenplum, S3 и Airflow. Он служит основой для всех ключевых DAG'ов, обеспечивая стабильность, безопасность и централизованную логику.

---

#### 🔧 Основные возможности

1. **Интеграция с API CTL**:
   - `ctl_api()` — безопасный HTTP-клиент с повторными попытками, ограничением частоты вызовов (`rate_limit`) и автоматическим логированием.
...
3. **Управление объектами и кэшированием**:
   - `ctl_obj_load()`, `ctl_obj_save()`, `ctl_obj_etag()` — загрузка и сохранение объектов (JSON, YAML) в S3 и Airflow Variables с проверкой по MD5.
   - Ретраи при ошибках доступа к S3.

4. **Логирование и мониторинг**:
   - `add_note()` — структурированные заметки в интерфейсе Airflow.
   - `logging()` — унифицированное логирование с поддержкой уровней (info, error, warning).
   - Детальные логи выполнения SQL, API-вызовов и загрузок.

5. **Вспомогательные функции**:
   - `get_config()` — ленивая загрузка конфигурации из Airflow Variable `ctl_config`; результат кешируется на весь lifetime процесса. Используется вместо прямого обращения к модульной переменной.
   - `eval_delta()` — вычисление временных смещений (например, `+1 hour`, `weekday=1`).
   - `rate_limit()` — контроль частоты вызовов API (до 100 вызовов в секунду).
   - `category_recursive()` — построение иерархии задач по категориям CTL.

6. **Безопасность и отказоустойчивость**:
   - Все внешние вызовы защищены декоратором `@retry` (библиотека `tenacity`).
   - Обработка таймаутов, обрывов соединений, ошибок 5xx.
   - Пропуск 4xx ошибок с `AirflowSkipException` или `AirflowFailException`.
...
- **Ленивая инициализация конфигурации**:
  - Конфиг загружается при первом вызове `get_config()`, а не при импорте модуля.
  - Это предотвращает падение парсинга DAG-файлов при отсутствии Airflow Variable `ctl_config`.
  - Импортируйте `get_config`, а не `config`: `from plugins.ctl_utils import get_config`.

- **Повторные попытки (retry)**:
  - Используется `tenacity` с экспоненциальной задержкой.
  - Настройка: до 3 попыток, паузы от 1 до 30 сек.
  - Логирование каждой попытки через `log_retry_attempt`.

- **Ограничение нагрузки**:
  - `rate_limit(pool_name)` — предотвращает перегрузку API CTL.
  - Глобальный лимит: 100 вызовов в секунду.

- **Потоковая обработка**:
  - Поддержка ZIP/GZIP без распаковки на диск.
  - Использование `s3_IterStream` для защиты от OOM.

- **Интеграция с Airflow**:
  - Полная совместимость с Airflow 2.x.
  - Использование `@provide_session`, XCom, Variables, Hooks.
...
|--------|----------------|
| `get_config` | Все DAG'и — вместо прямого импорта `config` |
| `ctl_api`, `ctl_obj_load` | `ctl_loader`, `ctl_monitor`, `ctl_sensor` |
| `gp_exe`, `gp_upload_s3_csv` | `ctl_loader`, `ctl_tfs` |
| `rate_limit`, `pg_exe` | Все DAG'и с высокой нагрузкой на API/БД |
| `eval_delta` | Расчёт SLA, временных окон в мониторинге |

---

> 💡 *Примечание:* Модуль оптимизирован для production-нагрузки.
> Все функции протестированы на отказоустойчивость и безопасность.
> Предназначен для повторного использования в других компонентах ETL.
"""
