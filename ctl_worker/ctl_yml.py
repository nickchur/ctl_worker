...
Этот DAG предназначен для **экспорта всей конфигурации CTL** в формате YAML для бэкапа и миграции между окружениями.

---

#### 🔧 Основные функции

1. **Экспорт категорий** и сущностей в YAML.
2. **Экспорт workflow'ов** с параметрами и расписанием.
3. **Поддержка "безопасного" режима** (safe) для экспорта.
4. **Сохранение** в S3 и локальные файлы.
...
- **Пул выполнения:** `default_pool`.
- **Формат:** YAML для читаемости.
...
| Задача | Описание |
|-------|----------|
| `export_categories` | Экспортирует категории и сущности. |
| `export_workflows` | Экспортирует workflow'ы с параметрами. |

---

#### 🏷️ Метаданные

- **ID DAG:** `CTL.<profile>.yml`
...
2. Выберите режим (`safe` или полный экспорт).
3. Проверьте результат в S3 бакете.
"""
