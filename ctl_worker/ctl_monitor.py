"""### 📊 DAG: Мониторинг CTL (Change Tracking & Loading)

Этот DAG выполняет **автоматический мониторинг состояния загрузок** в системе CTL каждые 15 минут. Он анализирует активные загрузки, проверяет соблюдение SLA и выполняет автоматические действия при необходимости.

---

#### 🔧 Основные функции

1. **Периодический опрос** всех активных загрузок по категориям.
2. **Анализ статусов** и принятие решений:
   - `reRunned` — повторная попытка при ошибке.
   - `Aborted` — остановка при исчерпании попыток.
   - `Completed` — успешное завершение.
   - `reStarted` — перезапуск зависших задач.
   - `Skipped` — пропуск без изменений.
   - `SLA` — нарушение времени выполнения.
3. **Обновление статусов** в CTL через API.
4. **Управление расписанием** (удаление/создание scheduled).
5. **Логирование** всех действий в Airflow и CTL.
...
- **Пул выполнения:** `ctl_pool` для мониторинга, `pg_pool` для действий.
- **Логирование:** таблицы действий и статусов через `add_note`.
- **Интеграция:** взаимодействие с внешней системой CTL через `ctl_api`.
- **SLA-контроль:** автоматическое определение нарушений по `wf_interval`.
...
| Задача | Описание |
|-------|----------|
| `ctl_monitor` | Анализирует состояние всех загрузок и определяет действия. |
| `ctl_action` | Выполняет автоматические действия над загрузками. |

---

#### 🏷️ Метаданные

- **ID DAG:** `CTL.<profile>.monitor`
- **Теги:** `['CTL', '<profile>', 'CTL_agent', 'logger']`
- **Расписание:** каждые 15 минут
- **Catchup:** `False`
- **Max active runs:** `1`
- **Таймаут:** `10 минут`
- **Пул:** `ctl_pool` (мониторинг), `pg_pool` (действия)
...
| 📋 | `PREREQ` | Ожидание предусловий |
| 🔐 | `LOCK` | Блокировка |
| ⚙️ | `PARAM` | Настройка параметров |
| 🚀 | `START` | Запуск |
| 🔵 | `RUNNING` | Выполнение |
| ✅ | `SUCCESS` | Успех |
| ❌ | `ERROR` | Ошибка |
| 🔄 | `ERRORCHECK` | Проверка ошибки |
| 🛑 | `ABORTING` | Прерывание |
"""
