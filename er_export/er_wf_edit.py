...
рисует поле поиска с `datalist` и подсказкой «Start typing to see proposal values» —
пока не начнёшь печатать, список выглядит пустым. `enum` рендерится настоящим `<select>`
с поиском. Плата за это — выбрать можно только из списка: запись, созданная после
последнего синка, станет доступной для правки после ближайшего `export_er_sync`.
...
DAG при недоступности базы. Поэтому только что созданная кем-то запись появится в списке
после ближайшего синка; передать её ключ мимо списка, через conf, не выйдет — `enum`
проверяется схемой параметра и чужое значение отвергается.
