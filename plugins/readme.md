...
*2026-09-18 13:30 MSK · v1.7 · Nick Churkin · [NSChurkin@sber.ru](mailto:NSChurkin@sber.ru)*
...
- **`saved_params` / `store_params`** — Значения по умолчанию из Variable и сохранение
  ...
  Аргумент `extra` дописывает в переменную служебные ключи, которых нет в форме (например
  версию набора параметров у `tools_db_cleanup`): без него форма затирала бы их при каждом
  сохранении, потому что пишется она целиком. В сравнение «было → стало» такие ключи не
  входят — иначе первый же запуск у каждого тула показал бы ложное изменение
