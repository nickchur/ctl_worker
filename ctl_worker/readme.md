...
*2026-09-04 13:31 MSK · v1.12 · Nick Churkin · [NSChurkin@sber.ru](mailto:NSChurkin@sber.ru)*
...
| Ран у запаузенного дага | `QUEUED`, а даг на паузе: запуск дага паузу не проверяет, поэтому ран создаётся в очереди и не поедет, пока паузу не снимут |
...
призракам — даг пропадает из сериализации и на время сбоя разбора, — и к запаузенным:
паузу ставят на время работ, и свежий ран трогать нельзя, снимут паузу — он поедет сам. Параметр формы
