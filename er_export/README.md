...
Каждый пакет выгрузки представляет собой ZIP-архив с набором файлов. Требования согласованы с КАП (Полозов А.А., нояб. 2025).
...
| META-описание | совпадает с именем CSV, расширение `.meta` |
| TKT внутри архива | `[replica]__[YYYYMMDDHHMISS]__[any].tkt` — список имён CSV-файлов |
| TKT снаружи архива | то же — содержит имя ZIP-архива |

### CSV

- Разделитель: `;` (точка с запятой)
- Кодировка: UTF-8
- Текстовые поля в двойных кавычках
- Первая строка — заголовок
- Обязательные технические поля: `ctl_action` (I/U/D), `ctl_validfrom`
...
> `mask_file` — регулярное выражение для маски имени CSV; `null` если имя META совпадает с именем CSV.  
> `UK` — массив массивов (каждый элемент — один уникальный индекс).  
> Поля `params` в META-файле заполняются только при отклонении от дефолтов (нестандартный разделитель, кодировка и т.п.).

### TKT

- Одно имя файла на строку, без лишних символов, UTF-8.
- TKT **внутри** архива: перечисляет все CSV-файлы пакета.
- TKT **снаружи** архива: содержит только имя ZIP-файла (передаётся в TFS вместе с архивом).
