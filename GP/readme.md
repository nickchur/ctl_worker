...
*2026-09-03 09:40 MSK · v1.3 · Nick Churkin · [NSChurkin@sber.ru](mailto:NSChurkin@sber.ru)*
...
`statement_timeout` ставит не Greenplum и не CTL, а `gp_exe` (`plugins/ctl_utils.py:298`)
на время сессии. Значение приходит из `wf_timeout` воркфлоу, а при его отсутствии — из
`exe_timeout` в `ctl_config` (по умолчанию 4 часа). То есть письмо
`[query_canceled or statement_timeout]` означает «загрузка не уложилась в отведённое ей
время», и лечится оно либо ускорением SQL, либо `wf_timeout` у конкретного воркфлоу, а
не настройками базы.
